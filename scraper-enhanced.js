...
/**
 * Enhanced stealth configuration for better bot evasion
 */
function getStealthContext() {
  return {
    ...
    geolocation: { latitude: 41.8781, longitude: -87.6298 }, // Chicago coords
...
async function scrapeFrontierDirect(origin, destination, date, options = {}) {
  ...
  const testMode = options.testMode || false; // NEW: Test without proxy

  if (!testMode && !proxyManager) {
    throw new Error('proxyManager is required in options (or set testMode: true)');
  }
  ...
  while (attempts < maxRetries) {
    ...
    if (!testMode) {
      ...
      console.log('🧪 TEST MODE: No proxy being used');
    }
    ...
    try {
      // ========== STEP 1: Launch Browser with Enhanced Stealth ==========
      ...
      // Add proxy if not in test mode
      if (!testMode && selectedProxy) {
        launchOptions.proxy = {
      ...
      const stealthConfig = getStealthContext();
      ...
      if (status === 403) {
        console.log('❌ 403 Forbidden - Proxy detected by PerimeterX');
      ...
      try {
        ...
        if (botDetected.detected) {
          ...
          const proxyLabel = testMode ? 'no_proxy' : selectedProxy.proxy.replace(/[:.]/g, '_');
          const debugFilename = path.join(debugDir, `bot_detected_${proxyLabel}_${Date.now()}.html`);
      ...
      return {
        flights,
        proxyUsed: testMode ? 'TEST_MODE_NO_PROXY' : selectedProxy.proxy
      };
